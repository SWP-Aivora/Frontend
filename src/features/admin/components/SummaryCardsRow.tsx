...
export const SummaryCardsRow = ({ summary }: SummaryCardsRowProps) => (
  <div className="w-full overflow-x-hidden pb-4">
    <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-6">
      <div className="min-w-0 bg-white border border-slate-100 rounded-lg p-5 shadow-sm h-[240px] flex flex-col group hover:border-primary/20 transition-all">
      ...
      <div className="min-w-0 bg-white border border-slate-100 rounded-lg p-5 shadow-sm h-[240px] flex flex-col group hover:border-primary/20 transition-all">
        ...
        <div className="flex-1 overflow-x-hidden overflow-y-auto pr-1 custom-scrollbar space-y-2.5">
          {summary?.topCategories && summary.topCategories.length > 0 ? (
            <>
              ...
              {summary.topCategories.map((domain) => (
                <div key={domain.name} className="flex items-center justify-between group/item min-w-0 gap-2">
            ...
            <>
              ...
              {summary._rawJobs.map((job, idx) => (
                <div key={idx} className="flex items-center justify-between group/item min-w-0 gap-2">
      ...
      <div className="min-w-0 bg-white border border-slate-100 rounded-lg p-5 shadow-sm h-[240px] flex flex-col group hover:border-primary/20 transition-all">
        ...
        <div className="flex-1 flex flex-col gap-4 overflow-x-hidden overflow-y-auto custom-scrollbar">
          {(summary?.openDisputes || 0) > 0 ? (
            <>
              <div className="flex items-center gap-3 p-3 bg-rose-50/50 rounded-lg border border-rose-100/50 min-w-0">
      ...
      <div className="min-w-0 bg-white border border-slate-100 rounded-lg p-5 shadow-sm h-[240px] flex flex-col group hover:border-primary/20 transition-all">
        ...
        <div className="flex-1 overflow-x-hidden overflow-y-auto pr-1 custom-scrollbar divide-y divide-slate-50">
          {summary?.healthAlerts && summary.healthAlerts.length > 0 ? (
            summary.healthAlerts.map((alert, idx) => (
              <div key={idx} className="flex gap-4 py-3 group/item min-w-0">
                <div
                  className={cn(
