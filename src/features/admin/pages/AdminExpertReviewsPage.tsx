...
export const AdminExpertReviewsPage = () => {
  ...
  const { data: realData, isLoading, isError, error } = useAdminExpertReviews();
  ...
  return (
    <div className="space-y-4 pb-10">
      ...
      <div className="bg-white border border-slate-100 rounded-xl p-4 shadow-sm flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        <div>
          <p className="text-slate-500 text-[11px] font-medium mb-1">Admin / Expert Verifications</p>
      ...
      <div className="bg-primary border border-primary-dark rounded-2xl p-4 lg:p-5 flex flex-col lg:flex-row justify-between relative overflow-hidden shadow-sm">
        ...
        <div className="relative z-10 lg:w-1/2 flex flex-col justify-between mt-6 lg:mt-0">
          ...
          <div className="flex flex-wrap gap-2">
            <div className="bg-white text-primary px-3 py-1 rounded-full text-[10px] font-semibold">{data?.totalPending || '0'} pending</div>
            <div className="bg-rose-50 text-rose-600 px-3 py-1 rounded-full text-[10px] font-semibold">{data?.totalRejected || '0'} rejected</div>
            <div className="bg-orange-50 text-orange-600 px-3 py-1 rounded-full text-[10px] font-semibold">{data?.totalRevisions || '0'} revisions</div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white border border-slate-100 rounded-xl shadow-sm flex flex-col">
        <div className="p-5 border-b border-slate-50 flex flex-col sm:flex-row justify-between items-center gap-4">
          ...
          <div className="flex items-center gap-4">
            ...
            <span className="text-[11px] font-medium text-slate-500">Page 1 of 2 • {filteredReviews.length} total</span>
          </div>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-slate-50/50">
              <tr>
                <th className="px-4 py-2 text-left text-[10px] font-bold text-slate-500 uppercase tracking-wide">Expert</th>
                <th className="px-4 py-2 text-left text-[10px] font-bold text-slate-500 uppercase tracking-wide">Title</th>
                <th className="px-4 py-2 text-left text-[10px] font-bold text-slate-500 uppercase tracking-wide">Skills</th>
                <th className="px-4 py-2 text-left text-[10px] font-bold text-slate-500 uppercase tracking-wide">Exp</th>
                <th className="px-4 py-2 text-left text-[10px] font-bold text-slate-500 uppercase tracking-wide">Proof</th>
                <th className="px-4 py-2 text-left text-[10px] font-bold text-slate-500 uppercase tracking-wide">Status</th>
                <th className="px-4 py-2 text-left text-[10px] font-bold text-slate-500 uppercase tracking-wide">Submitted</th>
                <th className="px-4 py-2 text-center text-[10px] font-bold text-slate-500 uppercase tracking-wide">Action</th>
              </tr>
            </thead>
                ...
                >
                  <td className="px-4 py-2.5">
                    <div className="flex items-center gap-3">
                      <div className="size-8 rounded-full bg-primary flex items-center justify-center text-[9px] font-bold text-white">
                        {rev.initials}
                      </div>
                      <div>
                        <p className="text-xs font-semibold text-slate-900 group-hover:text-primary transition-colors">{rev.fullName}</p>
                        <p className="text-[9px] text-slate-500 font-normal">{rev.email}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-4 py-2.5 whitespace-nowrap text-[10px] font-medium text-slate-700">
                    {rev.title}
                  </td>
                  <td className="px-4 py-2.5">
                  ...
                  <td className="px-4 py-2.5 text-[10px] font-bold text-slate-700 text-center">
                  ...
                  <td className="px-4 py-2.5 text-center">
                    <button className="bg-primary text-white px-3 py-1 rounded-full text-[10px] font-bold hover:bg-primary-dark transition-colors">
                      Review
                    </button>
                  </td>
                </tr>
        ...
        <div className="p-4 border-t border-slate-50 bg-slate-50/30 flex items-center justify-center gap-4">
           ...
           <span className="text-[9px] font-semibold text-slate-500 tracking-widest uppercase">Page 1 of 2</span>
