...
import { DisputeErrorBoundary } from '../components/DisputeErrorBoundary';
  ...
  if (error || !response?.data) return (
    <div className="flex flex-col items-center justify-center min-h-[80vh] p-8 text-center bg-[#f0f4f9]">
      ...
      <p className="text-slate-500 max-w-md mb-8">
        {error ? 'Unable to load dispute details. Please try again.' : 'The requested dispute case could not be retrieved from the server.'}
      </p>
      <Button onClick={() => navigate(-1)} variant="outline" className="rounded-xl px-8">
        <ArrowLeft className="size-4 mr-2" />
        Go Back
            ...
            <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-6">
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center justify-between p-3 bg-blue-50 rounded-xl border border-blue-100">
                  ...
                  <span className="text-sm font-black text-slate-900">{dispute.expertName}</span>
                </div>
              </div>
            </div>
